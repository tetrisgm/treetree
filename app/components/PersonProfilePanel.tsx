...
export default function PersonProfilePanel({ person, tree, canEdit, canComment, onClose, onSelect, onTreeChange, preview, meId }: Props) {
  ...
  /* Picking a name from the suggestions is the decision; asking for a second
     click to confirm it only ever looked broken. `chosen` is that person, so
     the add does not depend on state React has not committed yet. */
  async function addRelative(label: string, chosen?: Person) {
    if ((!chosen && !relativeQuery.trim()) || saving) return;
    setSaving(true); setNotice("");
    try {
      let relative = chosen ?? tree.people.find((candidate) => candidate.id === relativeChoice);
      if (!relative) {
      ...
      /* Siblings are recorded as shared parents, so the link runs from
         whichever of the two has parents on file - usually this person, but
         when the record being filled in is the empty one it is the sibling
         who supplies them. */
      const siblingLinks = () => {
        if (buckets.parents.length) return buckets.parents.map((parent) => ({ fromPersonId: parent.id, toPersonId: relative!.id }));
        const theirs = relatedPeople(tree, relative!.id).parents;
        return theirs.map((parent) => ({ fromPersonId: parent.id, toPersonId: person.id }));
      };
      const links = label === "Parents" ? [{ fromPersonId: relative.id, toPersonId: person.id }]
        ...
        : siblingLinks();
      if (!links.length) throw new Error(`Neither ${person.displayName} nor ${relative.displayName} has a parent recorded, and a sibling is recorded as a shared parent. Add a parent to either of them first.`);
