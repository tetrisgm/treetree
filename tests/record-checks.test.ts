...
describe("record checks", () => {
  ...
  it("credits a story's claim to its subject only", () => {
    const checks = runRecordChecks(tree(
      [person("m", "Arthur Rowan", { birthDate: "1923", deathDate: "1991" }), person("h", "Leo Rowan", { birthDate: "1950", deathDate: "2020" })],
      [],
      [{ id: "s", title: "Arthur Rowan, first generation", body: "He was over a hundred years old. Leo tended the garden.", date: null, place: null, personIds: ["m", "h"] }],
    ));
  ...
  it("asks about same-name records only when the archive cannot tell them apart", () => {
    const ambiguous = runRecordChecks(tree(
      [person("a", "Owen Rowan"), person("b", "Owen Rowan"), person("p", "A Parent")],
      [parent("p", "a")],
    ));
    expect(ambiguous.map((check) => check.kind)).toEqual(["duplicate"]);

    const distinguished = runRecordChecks(tree(
      [person("a", "Owen Rowan"), person("b", "Owen Rowan"), person("p1", "One"), person("p2", "Two")],
