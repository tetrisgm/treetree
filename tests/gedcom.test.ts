...
const tree: FamilyTree = {
  people: [
    person("f", "Arthur Rowan", { gender: "male", birthDate: "1923", deathDate: "1991-04-02", birthCity: "Bath", birthCountry: "United Kingdom", burialPlace: "Bath cemetery", residence: "Bristol, United Kingdom" }),
    person("m", "Évelyn Vale", { gender: "female" }),
    person("c", "Leo Rowan", { gender: "male", birthDate: "1950", biography: "Tended a community garden." }),
  ],
  relationships: [
  ...
  stories: [{ id: "s", title: "A day in the garden", body: "A fictional afternoon planting apple trees.", date: "1976", place: null, personIds: ["c"] }],
};
...
describe("gedcom export", () => {
  ...
  it("exports where a person lived as a residence event", () => {
    ...
    expect(text).toContain("2 PLAC Bristol, United Kingdom");
  });

  it("writes names, sex, dates and places", () => {
    expect(text).toContain("1 NAME Arthur /Rowan/");
    expect(text).toContain("1 SEX M");
    expect(text).toContain("2 DATE 1923");
    expect(text).toContain("2 DATE 2 APR 1991");
    expect(text).toContain("2 PLAC Bath, United Kingdom");
  ...
  it("carries biographies and the stories a person appears in", () => {
    expect(text).toContain("1 NOTE Tended a community garden.");
    expect(text).toContain("A day in the garden (1976)");
  });
});
